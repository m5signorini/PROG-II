...
void aux_node_destroy(void* n);
void * aux_node_copy(const void* n);
int aux_node_print(FILE *pf, const void* n);
...
Node *graph_findDeepSearch (Graph *g, int from_id, int to_id) {
    ...
    st = stack_ini(aux_node_destroy, aux_node_copy, aux_node_print);
    if(st == NULL) return NULL;
    
    nu = graph_getNode(g, from_id);
    if(nu == NULL) {
        stack_destroy(st);
        return NULL;
    }
    ...
    while(stack_isEmpty(st) == FALSE) {
        ...
        if(node_getEtq(nu) == BLANCO) {
            ...
            id_act = node_getId(nu);
            
            tam = graph_getNumberOfConnectionsFrom(g, id_act);
            ids = graph_getConnectionsFrom(g, id_act);
            if(ids == NULL && tam > 0) {
                node_destroy(nu);
                node_destroy(nw);
                stack_destroy(st);
            ...
            for(i = 0; i < tam; i++) {
                // Obtengo el id
                id_act = ids[i];
                // Miro si es el id destino
                if(id_act == to_id) {
                    node_destroy(nu);
                    stack_destroy(st);
                    free(ids);
                    return graph_getNode(g, id_act);
                }
                else if(id_act == BLANCO) {
                    // Obtengo nodo
                    nw = graph_getNode(g, id_act);
                    if(nw == NULL) {
                        node_destroy(nu);
                        stack_destroy(st);
                        free(ids);
