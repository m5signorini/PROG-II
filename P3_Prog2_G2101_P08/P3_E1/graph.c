 ...
 * Nombre: graph.c
 *
 * Descripción: Estructura Grafo (conjunto de nodos) adaptada a la practica 3
 *
...
void aux_dbs_free(Queue *q, int* pindex, int* conexs);
...
Node *graph_findDeepSearch (Graph *g, const int from_id, const int to_id) {
    ...
    q = queue_ini(aux_int_destroy, aux_int_copy, aux_int_print);
    if(q == NULL) return NULL;

    // Push dicho indice en la pila
    if(queue_insert(q, &index) == ERROR) {
        queue_destroy(q);
        return NULL;
    }
        ...
        if(node_getEtq(nu) == BLANCO) {
            ...
            for(i = 0; i < tam; i++) {
                ...
                if(node_getEtq(nw) == BLANCO) {
                    ...
                    node_setAntId(nw, index);
                    // PUSH
                    if(queue_insert(q, conexs + i) == ERROR) {
                        aux_dbs_free(q, pindex, conexs);
                        return NULL;
                    }
                }
